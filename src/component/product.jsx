...
import { dataProduct } from './libs'
import Image from 'next/image'

const product = () => {
    return (
        <>
            <section className='flex w-full justify-center items-center py-24 bg-dark' id='product'>
                <div className='container flex flex-col justify-center items-center max-w-full gap-10'>
                    ...
                    <div className="container grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                        {dataProduct.slice(0,3).map((card) => {
                            return (
                            <div key={card.id} className="mb-10 overflow-hidden rounded-lg bg-white shadow-1 duration-300 hover:shadow-3 dark:bg-dark-2 dark:shadow-card dark:hover:shadow-3">
                                ...
                                <div className="p-8 text-center sm:p-9 md:p-7 xl:p-9">
                                    ...
                                    <div className='flex justify-around items-center'>
                                        ...
                                        <a href={`/product/${card.id}`} className="inline-block rounded-full border border-gray-3 px-7 py-2 text-base font-medium text-body-color transition hover:border-primary hover:bg-primary hover:text-white dark:border-dark-3 dark:text-dark-6">
                                            Lihat Detail
                                        </a>
                                    </div>
                                </div>
                    ...
                    <a href='/product' className="inline-block -mt-10 lg:-mt-4 rounded-full border border-gray-3 px-7 py-2 text-base font-medium text-body-color transition hover:border-primary hover:bg-primary hover:text-white dark:border-dark-3 dark:text-dark-6">
                        Lihat Semua
                    </a>
                </div>
            </section>
